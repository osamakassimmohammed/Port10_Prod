...
<div class="form-group">
  ...
  <?php if (!empty($subq)) { ?>
    <?php if ($users_data[0]['subs_status'] == 'expired') { ?>
      <span>
        <?php echo lang('aYour_Plan_Status'); ?>:<span class="label label-danger">
          <?php echo lang('aExpired'); ?>
        </span>
      </span>
    <?php } else if ($users_data[0]['subs_status'] == 'trial') { ?>
        <span>
        <?php echo lang('aYour_Plan_Status'); ?>:<span class="label label-primary">
          <?php echo lang('aTrial'); ?>
          </span>
        </span>
    <?php } else { ?>
        <span>
        <?php echo lang('aYour_Plan_Status'); ?>:<span class="label label-success">
          <?php echo lang('aActive'); ?>
          </span>
        </span>
    <?php } ?>
  <?php } ?>
...
<table class="table table-bordered table-striped table-hover dataTable js-exportable">
  <thead>
    <tr>
      ...
      <th>
        <?php echo lang('aDate_of_Payment'); ?>
      </th>
      <th>
        <?php echo lang('aStart_Date'); ?>
      </th>
      <th>
        <?php echo lang('aEnd_Date'); ?>
      </th>
      <!-- <th>Error</th>       -->
  ...
  <tbody id="table_body">
    ...
    if (!empty($trans_data)) {
      foreach ($trans_data as $row) { ?>
        ...
        <td>
          <?php echo $row['subs_start_date']; ?>
        </td>
        <td>
          <?php echo $row['subs_end_date']; ?>
        </td>
...
<?php if (!empty($subq)) { ?>
  <a class="sub_btn" style="float: right;margin-bottom: 10px;" href="<?php echo base_url($language . '/price') ?>"
    class="btn bg-light-green waves-effect"><span>
      <?php echo lang('aRenew_Subscription'); ?>
    </span></a>
<?php } ?>
<div id="pagination">
  <?php echo @$pagination; ?>
