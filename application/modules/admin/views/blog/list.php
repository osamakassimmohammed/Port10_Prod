...
//print_r($list);
  if (!empty($blog_data)){ foreach($blog_data as $key => $row) {?>

<tr>
<td><?php echo $key+1; ?></td>
<td>
<a target="_blank" href="<?php echo base_url('assets/admin/blog/').$row['image']?>" class="image-thumbnail"><img class="image-thumbnail" src="<?php echo base_url('assets/admin/blog/').$row['image']?>" height="50px"></a>
</td>
...
<td>
   <a style="width: 100%; background-color: #4f0381 !important;" href="blog/edit/<?php echo $row['id'] ?>" class="btn bg-light-green btn-circle waves-effect waves-circle waves-float " role="button"><i class="material-icons">edit</i><span class="material_label"><?php echo lang('aEdit'); ?></span></a>
   <a style="width: 100%; margin: 8px 0px !important; background-color: #44871b !important; " href="blog/tedit/<?php echo $row['id'] ?>" class="btn bg-light-green btn-circle waves-effect waves-circle waves-float " role="button">  عديل   <i class="material-icons">translate</i></a>
...
<script type="text/javascript">
    ...
    $('#loading').show();     
      $.ajax({
           type: 'POST',
           url: "<?php echo base_url("admin/blog/index"); ?>",
      ...
      swal({
          ...
          function(inputValue){         
            if (inputValue===true) {                    
              $.ajax({
              type: 'POST',
              url: '<?php echo base_url('admin/blog/detete_blog') ?>',
              data: {pid:pid},
              success: function(response){                               
               if(response)
               {
