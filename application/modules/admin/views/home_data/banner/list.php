...
//print_r($list);
  if (!empty($banner_data)){ foreach($banner_data as $key => $row) {?>

<tr>
<td><?php echo $key+1; ?></td>
<td>
<a target="_blank" href="<?php echo base_url('assets/admin/banner/').$row['image']?>" class="image-thumbnail"><img class="image-thumbnail" src="<?php echo base_url('assets/admin/banner/').$row['image']?>" height="50px"></a>
</td>
<td><?php echo $row['heading1']?></td>
<td><?php echo $row['display_name']?></td>
<td><?php echo $row['status']?></td>
<td>
   <a style="width: 100%; background-color: #4f0381 !important;" href="home_data/banner_edit/<?php echo $row['id'] ?>" class="btn bg-light-green btn-circle waves-effect waves-circle waves-float " role="button"><i class="material-icons">edit</i><span class="material_label"><?php echo lang('aEdit'); ?></span></a>
   <a style="width: 100%; margin: 8px 0px !important; background-color: #44871b !important; " href="home_data/banner_tedit/<?php echo $row['id'] ?>" class="btn bg-light-green btn-circle waves-effect waves-circle waves-float " role="button">  عديل     <i class="material-icons">translate</i></a>
...
<script type="text/javascript">
    ...
    $('#loading').show();     
      $.ajax({
           type: 'POST',
           url: "<?php echo base_url("admin/home_data/index"); ?>",
    ...
    // trHTML+='<td><img class="img_sa" width="100px" height="100px" src="<?php //echo base_url('assets/admin/products/') ?>'+v.product_image+' "></td>';
     ...
     function creatTable(tabledata,flag_row) {
      ...
      $.each(tabledata, function( k, v ) {   
           ...
           trHTML+='<td><a target="_blank" href="<?php echo base_url('assets/admin/banner/') ?>'+v.image+' " class="image-thumbnail"><img class="image-thumbnail" src="<?php echo base_url('assets/admin/banner/') ?>'+v.image+' " height="50px"></a></td>';         
           trHTML+='<td>'+v.heading1+'</td>';   
           trHTML+='<td>'+v.display_name+'</td>'; 
           trHTML+='<td>'+v.status+'</td>'; 
           trHTML+='<td><a href="<?php echo base_url();?>admin/home_data/banner_edit/'+v.id+'" class="btn bg-light-green btn-circle waves-effect waves-circle waves-float" role="button"><i class="material-icons">edit</i></a> <a href="<?php echo base_url();?>admin/home_data/banner_tedit/'+v.id+'" class="btn bg-light-green btn-circle waves-effect waves-circle waves-float" role="button"><i class="material-icons">translate</i></a> <a href="javascript:void(0)" class="btn bg-light-green btn-circle waves-effect waves-circle waves-float  detete_pro" data-id="'+v.id+'" role="button" ><i class="material-icons">delete</i></a></td>';
        trHTML+='</tr>';
        flag_row++;                                  
      });  
      return trHTML;    
      ...
      swal({
          ...
          function(inputValue){         
            if (inputValue===true) {                    
              $.ajax({
              type: 'POST',
              url: '<?php echo base_url('admin/home_data/detete_banner') ?>',
              data: {pid:pid},
              success: function(response){                               
               if(response)
               {
                swal("","Banner Delete successfully",'success');
