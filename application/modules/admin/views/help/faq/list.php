...
<!-- <a target="_blank" href="<?php //echo base_url('assets/admin/faq/').$row['image']?>" class="image-thumbnail"><img class="image-thumbnail" src="<?php //echo base_url('assets/admin/faq/').$row['image']?>" height="50px"></a> -->
<?php //} ?>
<!-- </td> -->
<?php

//print_r($list);
  if (!empty($faq_data)){ foreach($faq_data as $key => $row) {?>
...
<td>
   <a style="width: 100%; background-color: #4f0381 !important;" href="help/edit/<?php echo $row['id'] ?>" class="btn bg-light-green btn-circle waves-effect waves-circle waves-float fist_a" role="button"><i class="material-icons">edit</i><span class="material_label"><?php echo lang('aEdit'); ?></span></a>
   <a style="width: 100%; margin: 00px 0px !important; background-color: #44871b !important; " href="help/tedit/<?php echo $row['id'] ?>" class="btn bg-light-green btn-circle waves-effect waves-circle waves-float second_a" role="button">  عديل    <i class="material-icons">translate</i></a>

   <a style="width: 100%; background-color: #ff375e !important;" href="javascript:void(0)" class="btn bg-light-green btn-circle waves-effect waves-circle waves-float  detete_pro" data-id="<?php echo $row['id']; ?>" role="button" ><i class="material-icons">delete</i><span class="material_label"><?php echo lang('aDelete'); ?></span></a>
</td>
</tr>
<?php } } else { ?>
  <tr class="text-center text-danger"><td colspan="7" >No Record Found</td></tr>
<?php } ?>
</tbody>
</table>
...
<script type="text/javascript">
    ...
    $('#loading').show();     
      $.ajax({
           type: 'POST',
           url: "<?php echo base_url("admin/help/index"); ?>",
...
<script type="text/javascript">
    // trHTML+='<td><img class="img_sa" width="100px" height="100px" src="<?php //echo base_url('assets/admin/products/') ?>'+v.product_image+' "></td>';
     ...
     // trHTML+='<td><a href="<?php //echo base_url();?>admin/car_listing/detail/'+v.id+'" class="btn bg-light-green btn-circle waves-effect waves-circle waves-float" role="button"><i class="material-icons">remove_red_eye</i></a></td>'; 
      ...
      //      if(v.image!='')
      //      {
      //       trHTML+='<a target="_blank" href="<?php //echo base_url('assets/admin/faq/') ?>'+v.image+' " class="image-thumbnail"><img class="image-thumbnail" src="<?php //echo base_url('assets/admin/faq/') ?>'+v.image+' " height="50px"></a>';
      //      }
      //      trHTML+='</td>'; 
     function creatTable(tabledata,flag_row) {
      ...
      $.each(tabledata, function( k, v ) {   
           ...
           trHTML+='<td>'+v.question+'</td>';
           trHTML+='<td>'+v.answer+'</td>';; 
           trHTML+='<td><a style="width: 100%; background-color: #4f0381 !important;" href="help/edit/'+v.id+'" class="btn bg-light-green btn-circle waves-effect waves-circle waves-float fist_a" role="button"><i class="material-icons">edit</i><span class="material_label"><?php echo lang('aEdit'); ?></span></a>  <a style="width: 100%; margin: 00px 0px !important; background-color: #44871b !important; " href="help/tedit/'+v.id+'" class="btn bg-light-green btn-circle waves-effect waves-circle waves-float second_a" role="button">ديل <i class="material-icons">translate</i></a> <a style="width: 100%; background-color: #ff375e !important;" href="javascript:void(0)" class="btn bg-light-green btn-circle waves-effect waves-circle waves-float  detete_pro" data-id="'+v.id+'" role="button" ><i class="material-icons">delete</i><span class="material_label"><?php echo lang('aDelete'); ?></span></a></td>';
        trHTML+='</tr>';  
        flag_row++;                              
      });  
      return trHTML;    
      ...
      swal({
          ...
          function(inputValue){         
            if (inputValue===true) {                    
              $.ajax({
              type: 'POST',
              url: '<?php echo base_url('admin/help/delete_faq') ?>',
              data: {pid:pid},
              success: function(response){                               
               if(response)
               {
                swal("","Faq Delete successfully",'success');
