...
class Chat extends Admin_Controller
{
                        ...
                        if (!empty($is_seller)) {
                            ...
                            if ($seller_id != $post_data['user_id']) {
                                ...
                                if ($compose_id) {
                                    ...
                                    echo json_encode(array("status" => true, "message" => ($language == 'ar' ? 'تم إرسال الرسالة بنجاح' : 'Message sent successfully'), 'data' => $html_tag));
                                    die;
                                } else {
                                ...
                                echo json_encode(array("status" => false, "message" => ($language == 'ar' ? "لا يمكنك إنشاء رسالة لنفسك" : "You can't compose message to your self")));
                                die;
                            }
                        } else {
                            echo json_encode(array("status" => false, "message" => ($language == 'ar' ? "معرف البائع غير صحيح" : "Invalid seller id")));
                            die;
                        }
                    } else {
                        echo json_encode(array("status" => false, "message" => ($language == 'ar' ? "ل الحقول مطلوبة" : "All field required")));
                        die;
                    }
                } else {
                    echo json_encode(array("status" => false, "message" => ($language == 'ar' ? "ل الحقول مطلوبة" : "All field required")));
    ...
    public function compose_to_all($post_data)
    {
        ...
        if (!empty($user_data)) {
            ...
            echo json_encode(array("status" => true, "message" => ($language == 'ar' ? 'تم إرسال الرسالة بنجاح' : 'Message sent successfully'), 'data' => ''));
            die;
        } else {
            echo json_encode(array("status" => false, "message" => ($language == 'ar' ? 'ناك خطأ ما' : 'Something went wrong')));
            die;
                ...
                if ($post_data['message_type'] == 'text') {
                    ...
                    if (isset($_FILES['name']['name']) && $_FILES['name']['name'] != '') {
                        ...
                        if ($logo != false) {
                            ...
                            echo json_encode(array("status" => false, "message" => ($language == 'ar' ? 'يرجى تحديد صورة صالحة' : 'Please Select Valid Image')));
                            die;
                        }
                    } else {
                        echo json_encode(array("status" => false, "message" => ($language == 'ar' ? 'يرجى تحديد صورة صالحة' : 'Please Select Valid Image')));
