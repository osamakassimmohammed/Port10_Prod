...
class VirtualAccount extends Admin_Controller {
	...
	public function index($rowno=0,$ajax='call',$serach='')
	{
    	...
    	if($ajax=='call')
		{
			...
			{
				$vaccount_data = $this->custom_model->get_data_array("SELECT * FROM virtual_accounts WHERE user_id=".$this->nmUser_id." ORDER BY id ASC limit $rowno,$rowperpage ");	 

				$vaccount_count = $this->custom_model->get_data_array("SELECT COUNT(id) as vaccount_count FROM virtual_accounts WHERE user_id=".$this->nmUser_id." ORDER BY id ASC  ");				
			}else{

				$vaccount_data = $this->custom_model->get_data_array("SELECT virtual_accounts.*, seller.first_name FROM virtual_accounts INNER JOIN admin_users as seller ON virtual_accounts.user_id=seller.id  ORDER BY virtual_accounts.id ASC limit $rowno,$rowperpage ");

				$vaccount_count = $this->custom_model->get_data_array("SELECT COUNT(virtual_accounts.id) as vaccount_count FROM virtual_accounts INNER JOIN admin_users as seller ON virtual_accounts.user_id=seller.id ORDER BY virtual_accounts.id ASC ");
			}
   		}else 
   		{
			if(empty($serach))
			{
				...
				if($this->mUser->type=="suppler" || $this->mUser->type=="subsupplier")
				{
					$vaccount_data = $this->custom_model->get_data_array("SELECT * FROM virtual_accounts WHERE (brand_name LIKE '%$serach%' OR id LIKE '%$serach%') AND user_id=".$this->nmUser_id." ORDER BY id ASC limit $rowno,$rowperpage ");

					$vaccount_count = $this->custom_model->get_data_array("SELECT COUNT(id) as vaccount_count FROM virtual_accounts WHERE (brand_name LIKE '%$serach%' OR id LIKE '%$serach%') AND user_id=".$this->nmUser_id." ORDER BY id ASC  ");
				}else{

					$vaccount_data = $this->custom_model->get_data_array("SELECT virtual_accounts.*, seller.first_name FROM virtual_accounts INNER JOIN admin_users as seller ON virtual_accounts.user_id=seller.id WHERE virtual_accounts.brand_name LIKE '%$serach%' OR virtual_accounts.id LIKE '%$serach%' ORDER BY virtual_accounts.id ASC limit $rowno,$rowperpage ");

					$vaccount_count = $this->custom_model->get_data_array("SELECT COUNT(virtual_accounts.id) as vaccount_count FROM virtual_accounts INNER JOIN admin_users as seller ON virtual_accounts.user_id=seller.id WHERE virtual_accounts.brand_name LIKE '%$serach%' OR virtual_accounts.id LIKE '%$serach%' ORDER BY virtual_accounts.id ASC   ");
				}	
			}	
		}
		
		
		$config['base_url'] = base_url().'admin/virtual_account/index';
	    $config['use_page_numbers'] = TRUE;
	    $config['total_rows'] = $vaccount_count[0]['vaccount_count'];
	    ...
	    $data['result'] = $vaccount_data;
	    $data['row'] = $rowno;
	    $data['total_rows'] = $vaccount_count[0]['vaccount_count'];
		...
		$this->mViewData['vaccount_data'] = $vaccount_data;
		$this->mViewData['user_id'] = $this->nmUser_id;
		$this->render('virtual_account/va_list');

	}

	public function dashboard($rowno=0,$ajax='call',$serach='')
	{
		$language= $this->uri->segment(1);
		if($language=='en')
		{
		...
		// die;
		$config['base_url'] = base_url().'admin/virtual_account/va_dashboard';
	    $config['use_page_numbers'] = TRUE;
		...
		$this->render('virtual_account/va_dashboard');
