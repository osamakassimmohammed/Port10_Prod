    ...
    public function create_invoice($id = '')
    {
        ...
        if (!empty($post_data)) {
            ...
            if ($is_request[0]['quotation_status'] == 'Cancelled') {
                echo json_encode(array('status' => false, "message" => ($language == 'ar' ? 'تم إلغاء الطلب من قبل المستخدم' : 'Request Cancelled By user')));
                die;
            }
            ...
            if (empty($is_product)) {
                echo json_encode(array('status' => false, "message" => ($language == 'ar' ? 'معرف المنتج غير صحيح أو هذا المنتج لا ينتمي لك' : 'Invalid product id or this product not belongs to you')));
                die;
            }
            if (!empty($is_request)) {
                ...
                if ($is_request[0]['invoice_status'] == 'Cancelled') {
                    echo json_encode(array('status' => false, "message" => ($language == 'ar' ? ' المستخدم الغى الطلب' : 'User Cancelled Request')));
                    die;
                } else if ($is_request[0]['invoice_status'] == 'Rejected') {
                    echo json_encode(array('status' => false, "message" => ($language == 'ar' ? 'You Rejected Request' : 'You Rejected Request')));
                    die;
                } else if ($is_request[0]['invoice_status'] == 'Confirmed') {
                    echo json_encode(array('status' => false, "message" => ($language == 'ar' ? 'تم ارسال الفاتورة من قبل' : "You've already sent the invoice")));
                    die;
                } else {
                    ...
                    if ($response) {
                        ...
                        echo json_encode(array('status' => true, "message" => ($language == 'ar' ? 'م إرسال طلب التسعيرة بنجاح' : 'Request send successfully')));
                        die;
                    } else {
                        echo json_encode(array('status' => false, "message" => ($language == 'ar' ? 'هناك خطأ ما' : 'Something wnet wrong')));
                        die;
                    }
                }
            } else {
                echo json_encode(array('status' => false, "message" => ($language == 'ar' ? 'معرف طلب غير صحيح' : 'Invalid request id')));
                die;
            }
        }
    ...
    public function reject_request()
    {
        ...
        if (!empty($post_data)) {
            ...
            if (isset($post_data['req_id'])) {
                ...
                if (!empty($is_request)) {
                    if ($is_request[0]['quotation_status'] == 'Cancelled') {
                        echo json_encode(array('status' => false, "message" => "You Can't Reject Request already cancelled"));
                        die;
                    }
                    if ($is_request[0]['invoice_status'] == 'Rejected') {
                        echo json_encode(array('status' => false, "message" => "You already Rejected Request"));
                        die;
                    }
                    ...
                    if ($response) {
                        ...
                        echo json_encode(array('status' => true, "message" => "Request Rejected successfully"));
                        die;
                    } else {
