		...
		if(!empty($post_data))
		{
			...
			{	
				...
				if(!empty($post_data['first_name']) && !empty($post_data['username']) && !empty($post_data['street_name']) && !empty($post_data['building_no']) && !empty($post_data['city']) && !empty($post_data['state']) && !empty($post_data['postal_code']) && !empty($post_data['phone']) && !empty($post_data['email'])  && !empty($post_data['access_permission']) )
				{
					...
					if(!empty($is_username))
					{
						echo json_encode( array("status" => false,"message" => ($language == 'ar'? 'قم السجل التجاري مسجل!':'CR number already exist !'),"flag"=>"cr_number" ) );die;
					}
					...
					if(!empty($is_phone))
					{						
						echo json_encode( array("status" => false,"message" => ($language == 'ar'? 'رقم الهاتف المحمول مسجل!':'Mobile number already exist !'),"flag"=>"phone" ) );die;
					}					
					...
					if(!empty($is_email))
					{
						echo json_encode( array("status" => false,"message" => ($language == 'ar'? 'البريد الإلكتروني مسجل!':'Email already exist'),"flag"=>"email" ) );die;
					}				
					...
					if(!empty($post_data['access_permission']))
					{
						foreach ($post_data['access_permission'] as $key => $val) 
						{
						   ...
						   {
						   		echo json_encode( array("status" => false,"message" => ($language == 'ar'? 'Please Select Valid Access Permission':'Please Select Valid Access Permission'),"flag"=>"email" ) );die;
						   }
						   // else{
						   // }
						}
					...
					{	
						...
						if($response)
						{
							...
							echo json_encode( array("status" => true,"message" => ($language == 'ar'? 'تم التسجيل بنجاح':'Successfully Register'),"flag"=>"added" ) );die;
						}else{							
							echo json_encode( array("status" => false,"message" => ($language == 'ar'? 'هناك خطأ ما':'Something went wrong'),"flag"=>"something" ) );die;
						}
					}else{
						$response=$this->custom_model->my_update($update_data,array('id' => $euid,'seller_id'=>$this->mUser->id,'type'=>'subsupplier'),'admin_users');
						if($response)
						{
							echo json_encode( array("status" => true,"message" => ($language == 'ar'? 'تحديث الملف الشخصي بنجاح':'Profile updated successfully'),"flag"=>"updated" ) );die;
						}else{
							echo json_encode( array("status" => false,"message" => ($language == 'ar'? 'هناك خطأ ما':'Something went wrong'),"flag"=>"something" ) );die;
						}					
					}	
				}else{					
					echo json_encode( array("status" => false,"message" => ($language == 'ar'? 'كل الحقول مطلوبة':'All fields are required'),"flag"=>"all_fild_required" ) );die;
				}							
			}else{
				echo json_encode( array("status" => false,"message" => ($language == 'ar'? 'كل الحقول مطلوبة':'All fields are required'),"flag"=>"all_fild_required" ) );die;
			}
