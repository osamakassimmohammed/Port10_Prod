...
class Catalogue extends Admin_Controller {
	...
	public function list1($rowno=0,$ajax='call',$serach='')
	{
		...
		if($language=='en')
		{
			$err_msg1='Catalog List';
		}else{
			$err_msg1='قائمة بنجاح';
		}
    	...
    	if($ajax=='call')
		{
			...
			$product = $this->custom_model->get_data_array("SELECT pro.id,pro.is_active,pro.catalog_name,admin.first_name,pro.seller_products,pro.theme FROM catalog as pro INNER JOIN admin_users as admin ON pro.seller_id=admin.id  $sub_query  Order BY pro.id ASC limit $rowno,$rowperpage ");

			$product_count = $this->custom_model->get_data_array("SELECT  COUNT(pro.id) as product_count FROM catalog as pro INNER JOIN admin_users as admin ON pro.seller_id=admin.id  $sub_query  Order BY pro.id ASC");
   		}else{
			if(empty($serach))
			{
				$product = $this->custom_model->get_data_array("SELECT pro.id,pro.is_active,pro.catalog_name,admin.first_name,pro.seller_products,pro.theme FROM catalog as pro INNER JOIN admin_users as admin ON pro.seller_id=admin.id  $sub_query  Order BY pro.id ASC limit $rowno,$rowperpage ");
				...
				$product = $this->custom_model->get_data_array("SELECT pro.id,pro.is_active,pro.catalog_name,admin.first_name,pro.seller_products,pro.theme FROM catalog as pro INNER JOIN admin_users as admin ON pro.seller_id=admin.id  WHERE (pro.catalog_name LIKE '%$serach%' OR admin.first_name LIKE '%$serach%') $sub_query  Order BY pro.id  ASC limit $rowno,$rowperpage ");

				$product_count = $this->custom_model->get_data_array("SELECT COUNT(pro.id) as product_count FROM catalog as pro INNER JOIN admin_users as admin ON pro.seller_id=admin.id  WHERE (pro.catalog_name LIKE '%$serach%' OR admin.first_name LIKE '%$serach%') $sub_query  Order BY pro.id  ASC ");
			}
		}
		// print_r($product);
		// exit();
		...
		$config['base_url'] = base_url().'admin/catalogue/list1';
	...
	public function create()
	{
		...
		if($language=='en')
		{
			$err_msg1='Catalog created successfully';
			$err_msg2='Something went wrong';
			$err_msg3='Add Catalog';
		}else{
			$err_msg1='تم إنشاء الكتالوج بنجاح';
		...
		// exit();
		$products = $this->db->get_where('product', array('seller_id'=>$user_details[0]['id'], 'status'=>1))->result_array();
		// echo "<pre>";
		// print_r($products);
		// exit();
		$this->mViewData['seller_products'] = $products;
		...
		if (!empty($post_data))
		{
			...
			$count = 0;
				...
				if(!empty($post_data['seller_id']))
				{
					...
					if (!array_key_exists('seller_products', $post_data)) {
						
						$err_msgsm = 'Select products empty';
						$this->system_message->set_error($err_msgsm);
						redirect($language.'/admin/catalogue/create');
					}
					...
					$catalog_data = [
						'catalog_name'=> $post_data['catalog_name'],
						'description'=> $post_data['description'],
						'seller_id'=> $post_data['seller_id'],
						'seller_products'=> json_encode($post_data['seller_products']),
						'theme'=> $post_data['select_catalog_theme'],
						'image_gallery'=> $post_data['image_gallery'],
						'update_date'=> $post_data['update_date'],
					];

					
					$response = $this->custom_model->my_insert($catalog_data,'catalog');
					...
					if ($response)
					{
						...
						redirect($language.'/admin/catalogue/list1');
					}
					else
					{
		...
		$this->render('catalogue/create_catalogue');
	}
    ...
    // 	{
    // 		$pid=$post_data['pid'];
	// 		$catalog = $this->db->get_where('catalog',array('id'=>$pid))->result();
	// 		$seller_id = $catalog[0]->seller_id;
	// 	// 			print_r($catalog);
	// 	// exit();
	// 		if($catalog[0]->is_active == 0){
	// 			$this->custom_model->my_update(array("is_active" => '0'),array('seller_id' => $seller_id),'catalog');
	// 			$this->custom_model->my_update(array("is_active" => '1'),array('id' => $pid),'catalog');

	// 		} else {
	// 			$this->custom_model->my_update(array("is_active" => '0'),array('id' => $pid),'catalog');
	// 		}
	...
	public function activate_catalog()
    {
		...
		// exit();
			if($catalog[0]->is_active == 0){
				$this->custom_model->my_update(array("is_active" => '1'),array('id' => $pid),'catalog');
				$seller_catalogs = $this->db->get_where('catalog',array('seller_id'=>$catalog[0]->seller_id,'id !='=>$pid))->result();
				if(!empty($seller_catalogs)){
					foreach($seller_catalogs as $seller_catalog){
						$this->custom_model->my_update(array("is_active" => '0'),array('id' => $seller_catalog->id),'catalog');
					}
				}	
			} else {
				$seller_catalogs = $this->db->get_where('catalog',array('seller_id'=>$catalog[0]->seller_id,'id !='=>$pid))->result();
        		...
        		if (!empty($main_id))
        		{
        			...
        			if (!empty($s_listing))
        			{
        				...
        				foreach ($s_listing as $skey => $vsalue)
        				{
        					...
        					$sdata = 'Sub --> '.$vsalue['id'].' . '.$vsalue['display_name'];
