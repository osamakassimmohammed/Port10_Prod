...
class Vorders extends Admin_Controller
{
    ...
    public function create_pickup($order_id)
    {
        ...
        if (!empty($post_data)) {
                ...
                if (!empty($post_data['ShippingDateTime']) && !empty($post_data['DueDate'])) {
                    ...
                    if (!empty($is_order)) {
                        if (!empty($is_order[0]['shipping_id'])) {
                            echo json_encode(array("status" => false, "message" => ($language == 'ar' ? "You Can't resend request for shipping" : "You Can't resend request for shipping")));
                            die;
                        }
                        ...
                        if (empty($seller_data)) {
                            echo json_encode(array("status" => false, "message" => ($language == 'ar' ? 'Invalid seller id' : 'Invalid seller id')));
                            die;
                        }
                        ...
                        if (!empty($order_items)) {
                            ...
                            echo json_encode(array("status" => false, "message" => ($language == 'ar' ? 'Product items not found or product delivered by supplier' : 'Product items not found or product delivered by supplier')));
                            die;
                        }
                    } else {
                        echo json_encode(array("status" => false, "message" => ($language == 'ar' ? 'Invalid request' : 'Invalid request')));
                        die;
                    }
                } else {
                    echo json_encode(array("status" => false, "message" => ($language == 'ar' ? 'All fields required' : 'All fields required')));
                    die;
                }
            } else {
                echo json_encode(array("status" => false, "message" => ($language == 'ar' ? 'All fields required' : 'All fields required')));
            ...
            echo json_encode(array("status" => false, "message" => ($language == 'ar' ? 'Something Went Wrong' : 'Something Went Wrong')));
            die;
        }
        echo "<pre>";
        print_r($post_data);
