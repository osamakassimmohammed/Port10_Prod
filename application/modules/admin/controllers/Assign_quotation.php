		...
		if(!empty($post_data))
		{
			if(isset($post_data['assign_vender']) && !empty($post_data['assign_vender']) && isset($post_data['req_id']))
			{
				...
				if(!empty($is_request))
				{
					...
					{
						echo json_encode(array('status'=>false,"message"=>"Request cancelled by buyer so you ca't assign to seller")); die;
					}
				...
				echo json_encode(array("status"=>true,"message"=>"Request assign successfully")); die;							
				}else{
					echo json_encode(array("status"=>false,"message"=>"Invalid request id")); die;	
				}
			}else{
				echo json_encode(array("status"=>false,"message"=>"Please pass dealer id")); die;	
			}
		}else{
