...
class Home extends Admin_Controller
{
    ...
    public function sign_up()
    {
        ...
        if (!empty($mobile)) {
            ...
            if ($query == 'email') {
                echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? 'البريد الالكتروني موجود بالفعل' : 'Email already exists')));
                die;
            } elseif ($query == 'phone') {
                echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? 'رقم الهاتف موجود بالفعل' : 'Phone number already exists')));
                die;
            } elseif ($query == 'invite_code') {
                echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? 'رمز الدعوة غير صالح' : 'Invitation code is invalid')));
                die;
            } else {
                // echo "<pre>";
                ...
                echo json_encode(array("status" => true, "ws" => $ws, "data" => $user[0], "message" => ($language == 'ar' ? 'تم إنشاء الحساب بنجاح' : 'Account Created Successfully')));
                die;
            }
        } else {
            echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? 'الطلب غير ساري المفعول' : 'Invalid request')));
            die;
        }
    }
    ...
    public function check_user_login($language = 'en', $ws)
    {
        ...
        echo json_encode(array("status" => false, "message" => ($language == 'ar' ? "مصادقة غير صالحة." : 'Invalid authentication.'), "language" => $language, "ws" => $ws));
        die;
    }
    ...
    public function login()
    {
        ...
        // result
        if ($logged_in == 'error') {
            echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? 'يرجى الانتظار حتى يقوم المشرف في وقت ما بتنشيط حسابك قريبًا' : 'Please wait while the administrator will soon activate your account')));
            die;
        } elseif ($logged_in == 'error1') {
            echo json_encode(array("status" => false, "message" => ('you cant login as a ' . $type . '')));
        } elseif ($logged_in == 'password') {
            echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? 'الرجاء إدخال كلمة المرور الصحيحة' : 'Please enter correct password')));
            die;
        } else if ($logged_in) {
            $user = $logged_in;
            $user_id = $user[0]->id;
            ...
            echo json_encode(array("status" => true, "ws" => $ws, "data" => $u_details[0], "message" => ($language == 'ar' ? 'تسجيل الدخول بنجاح' : 'Login Successfully')));
            die;
        } else {
    ...
    public function forget_password()
    {
        ...
        if (empty($string)) {
            echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? 'يرجى إدخال عنوان البريد الإلكتروني' : 'Please enter an email address .')));
            die;
        } else {
            $this->load->model('User_model');
            ...
            if ($datas) {
                ...
                if (@$datas->social == 'facebook' || @$datas->social == 'gmail') {
                    echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? 'لا يمكنك تغيير كلمة المرور لأنك تسجيل الدخول من خلال الاجتماعية' : 'You cannot change the password because you are logged in with social media.')));
                    die;
                }
                ...
                echo json_encode(array("status" => true, "ws" => $ws, "message" => ($language == 'ar' ? 'يرجى التحقق من بريدك الإلكتروني لإعادة تعيين كلمة المرور.' : 'Please check your email to reset your password.')));
                die;
            } else {
                echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? 'البريد الإلكتروني غير موجود الرجاء إدخال بريد إلكتروني صالح' : 'Email does not exist. Please enter a valid email. ')));
                die;
            }

        }
        echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? 'حدث خطأ نرجوا المشرف اتصال' : 'Something went wrong, please contact your administrator')));
        die;
    }
    ...
    public function profile_info()
    {
        ...
        if (!empty($user_id)) {
            ...
            if (empty($user_check)) {
                echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? 'المستخدم غير موجود' : 'The user does not exist')));
                die;
            }
            ...
            if (!empty($email_check)) {
                echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? 'البريد الإلكتروني موجود بالفعل' : 'Email already exists')));
                die;
            }
            ...
            echo json_encode(array("status" => true, "ws" => $ws, "data" => $data[0], "message" => ($language == 'ar' ? 'الملف الشخصي تحديث بنجاح' : 'Profile updated successfully')));
            die;
        } else {
            echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? 'الطلب غير ساري المفعول' : 'Invalid request')));
            die;
    ...
    public function change_pass()
    {
        ...
        if (empty($password) && strlen($password) < 6) {
            echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? 'الرجاء إدخال كلمة المرور على الأقل 6 أحرف' : 'Please enter password atleast 6 character')));
            die;
        }
        ...
        if (!empty($logged_in)) {
            if (password_verify($current_password, $logged_in[0]['password'])) {
                ...
                echo json_encode(array("status" => true, "ws" => $ws, "message" => ($language == 'ar' ? 'تم تغيير كلمة المرور بنجاح' : 'Password changed successfully')));
                die;
            } else {
                echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? 'الرجاء إدخال كلمة المرور الحالية صالحة' : 'Please enter valid current password')));
                die;
            }
        } else {
            echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? 'المستخدم غير موجود' : 'The user does not exist')));
    ...
    public function get_pages()
    {
        ...
        if (isset($data[0])) {
            echo json_encode(array("status" => true, "data" => $data, "ws" => $ws, "message" => ($language == 'ar' ? 'بنجاح' : 'Successfully')));
            die;
        } else {
            echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? 'الطلب غير ساري المفعول' : 'Invalid request')));
            die;
    ...
    public function category_wise_product_list()
    {
        ...
        if (empty($pagination)) {
            echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? 'أضف ترقيم الصفحات' : 'Add pagination')));
            die;
        }
        ...
        if ($data) {
            ...
            echo json_encode(array("status" => false, "message" => ($language == 'ar' ? 'لايوجد سجل' : 'No record found'), "ws" => $ws));
            die;
        }
    }
    ...
    public function get_subcategory()
    {
        ...
        if (!empty($category_id)) {
            ...
            if (!empty($c_listing)) {
                ...
                echo json_encode(array("status" => true, "cart_count" => $count, "category_listing" => $c_listing, "ws" => $ws, "message" => ($language == 'ar' ? '' : 'Successfully')));
                die;
            }
            echo json_encode(array("status" => false, "cart_count" => $count, "ws" => $ws, "message" => ($language == 'ar' ? '' : 'No record found')));
            die;
        } else {
            echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? '' : 'Invalid request')));
            die;
        }
    }
    ...
    public function product_detail()
    {
        ...
        if (empty($product_detail)) {
            echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? 'المنتج غير نشط' : 'Product is deactive')));
            die;
        }
        ...
        echo json_encode(array("status" => true, "related_Procut" => $related_Procut, "data" => $product_detail[0], "ws" => $ws, "message" => ($language == 'ar' ? 'بنجاح' : 'Successfully')));
        die;
    }
    ...
    public function add_to_cart()
    {
            ...
            if ($type == 'add') {
                ...
                if ($response) {
                    ...
                    if ($response['message'] == 'invalid_size') {
                        echo json_encode(array("status" => false, "ws" => $ws, "message" => "Invalid Size Request!"));
                        die;
                    } else if ($response['message'] == 'product_is_deactive') {
                        echo json_encode(array("status" => true, "ws" => $ws, "message" => "Product is deactive!"));
                        die;
                    } else if ($response['message'] == 'founded') {
                        echo json_encode(array("status" => true, "ws" => $ws, "message" => "Added to cart successfully!", "cart_count" => $count));
                        die;
                    } else if ($response['message'] == 'quantity_notinstock') {
                        echo json_encode(array("status" => false, "ws" => $ws, "message" => "Product quantity is not in stock!"));
                        die;
                    } else if ($response['message'] == 'quantity_not_avilable') {
                        echo json_encode(array("status" => false, "ws" => $ws, "message" => "Quantity not available!"));
                        die;
                    } else if ($response['message'] == 'first_time_added_successfully') {
                        echo json_encode(array("status" => true, "ws" => $ws, "message" => ($language == 'ar' ? 'تمت الإضافة إلى السلة بنجاح' : 'Added to cart successfully!'), "cart_count" => $count));
                        die;
                    }
                } else {
                    $msg = ($language == 'ar' ? 'لا يوجد رصيد كاف لإضافة كمية ...' : 'Not enough stock to add quantity...');
                    echo json_encode(array("status" => false, "ws" => $ws, "message" => $msg));
                    die;
                }
            } elseif ($type == 'update') {
                $metadata = '';
                ...
                if ($response['message'] == 'cart_updated') {
                    echo json_encode(array("status" => true, "ws" => $ws, "message" => "Updated successfully", "cart_count" => $product_count));
                    die;
                }
                if ($response['message'] == 'quantity_not_avilable') {
                    echo json_encode(array("status" => true, "ws" => $ws, "message" => "Quantity not available", "cart_count" => $product_count));
                    die;
                }
                if ($response['message'] == 'not_added_tocart') {
                    echo json_encode(array("status" => true, "ws" => $ws, "message" => "Product not added to cart", "cart_count" => $product_count));
                    die;
                }
                ...
                if ($response['message'] == 'quantity_not_update_below_one') {
                    echo json_encode(array("status" => true, "ws" => $ws, "message" => ($language == 'ar' ? '' : 'Quantity is not updated below one. you have to remove that product from cart. ')));
                    die;
                }
            }
        }
        ...
        if ($type == 'remove') {
            ...
            $product_count = $this->view_cart_count($user_id);
            if ($response == '-1') {
                echo json_encode(array("status" => false, "ws" => $ws, "message" => "Invalid removed request", "cart_count" => $product_count));
                die;
            }
            // echo "<pre>";
            // print_r($response);
            // die;
            echo json_encode(array("status" => true, "ws" => $ws, "message" => "Removed successfully", "cart_count" => $product_count));
            die;
        }
    ...
    public function view_cart()
    {
        ...
        if (empty($data)) {
            echo json_encode(array("status" => true, "ws" => $ws, "data" => $data, "card_count" => $count, "message" => "Cart is empty"));
            die;
        }
        ...
        echo json_encode(array("status" => true, "total_amount" => $sum, "ws" => $ws, "data" => $data, "card_count" => $count, "message" => "Successfully"));
        die;
    }
    ...
    public function place_order()
    {
        ...
        if ($response) {
            echo json_encode(array("status" => true, "ws" => $ws, "data" => $response, "message" => ($language == 'ar' ? 'تم وضع الطلب بنجاح' : 'Order placed successfully')));
            die;
        } else {
            echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? 'الطلب غير ساري المفعول' : 'Invalid request')));
            die;
    ...
    public function add_to_wish_list()
    {
        ...
        if (!empty($user_id)) {
            ...
            if ($type != 'add') {
                echo json_encode(array("status" => true, "message" => ($language == 'ar' ? 'تمت إزالة المنتج من قائمة الأمنيات.' : 'The product has been removed from wishlist.'), "ws" => $ws, "wish_list" => $my_wish));
                die;
            }
            echo json_encode(array("status" => true, "message" => ($language == 'ar' ? 'تمت إضافة المنتج بنجاح إلى قائمة الأمنيات.' : 'The product has been successfully added to your Wishlist.'), "ws" => $ws, "wish_list" => $my_wish));
            die;
        } else {
            echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? 'يرجى تسجيل الدخول لإضافة منتجات في قائمة الأمنيات' : 'Please sign in to add products to wishlist.')));
            die;
        }
    }
    ...
    public function view_wish_list()
    {
        ...
        if (!empty($user_id)) {
            ...
            if (!empty($wish_arr) && !empty($wish_arr[0]['content'])) {
                ...
                if (empty($my_wish)) {
                    echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? 'قائمة الأمنيات فارغة' : 'The wish list is empty')));
                    die;
                }
                ...
                if ($data) {
                    echo json_encode(array("status" => true, "ws" => $ws, "message" => ($language == 'ar' ? 'بنجاح' : 'Successfully'), "data" => $data));
                    die;
                }
            } else {
                $data = new stdClass();
                echo json_encode(array("status" => false, "ws" => $ws, "data" => $data, "message" => ($language == 'ar' ? 'لم يتم العثور على المنتج' : 'No product found !')));
                die;
            }
        } else {
            echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? 'الطلب غير ساري المفعول' : 'Invalid request')));
    ...
    public function order_history_user()
    {
        ...
        if (!empty($uid)) {
            ...
            if (!empty($data)) {
                ...
                echo json_encode(array("status" => true, "ws" => $ws, "data" => $data, "message" => ($language == 'ar' ? 'بنجاح' : 'Successfully')));
                die;
            } else {
                echo json_encode(array("status" => false, "ws" => $ws, "data" => $data, "message" => ($language == 'ar' ? 'لم يتم العثور على أي طلب' : 'No order found')));
                die;
            }
        }
    }
    ...
    public function insert_user_rating()
    {
        ...
        echo json_encode(array("status" => true, "message" => ($language == 'ar' ? 'تمت إضافة تعليقك بنجاح ....' : 'Your review added successfully....')));
        die;
    }
    ...
    public function fcm_update()
    {
        ...
        if (!empty($user_id)) {
            ...
            if (empty($user_check)) {
                echo json_encode(array("status" => false, "ws" => $ws, "message" => ($language == 'ar' ? 'لم يتم العثور على أي طلب' : 'User not found')));
                die;
            }
            ...
            if (isset($FILES["type"])) {
                ...
                if ((($imageFileType == "gif") || ($imageFileType == "jpeg") || ($imageFileType == "jpg") || ($imageFileType == "png"))) {
                    ...
                    if (move_uploaded_file($FILES["tmp_name"], $target_file)) {
                        ...
                        echo json_encode(array("status" => true, "data" => $newFileName, "url" => base_url("assets/admin/usersdata/") . $newFileName));
                        die;
                    } else {
                        echo json_encode(array("status" => false, "data" => ($language == 'ar' ? 'حاول مرة اخرى.' : 'Please try again.')));
                        die;
                    }
                } else {
                    echo json_encode(array("status" => false, "data" => ($language == 'ar' ? 'الرجاء تحميل صورة صالحة.' : 'Please upload valid image.')));
                    die;
                }
            }
        } else {
