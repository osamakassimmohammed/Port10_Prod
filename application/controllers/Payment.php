    ...
    public function index()
    {
        ...
        if (!empty($post_data)) {
            if (empty($uid) || empty($type)) {
                echo json_encode(array("status" => false, "message" => "Something went wrong", "error_type" => "login"));
                die;
            } else if ($type == "buyer") {
                echo json_encode(array('status' => false, "message" => "Buyer not allowed here  ", 'error_type' => "not_allowed"));
                die;
                ...
                if (!empty($subs_plans)) {
                    ...
                    echo json_encode(array('status' => false, "message" => "Invalid plan selected", 'error_type' => ""));
                    die;
                }
            } else {
                echo json_encode(array('status' => false, "message" => "Something wnet wrong", 'error_type' => ""));
                die;
            }
        } else {
            echo json_encode(array("status" => false, "message" => "Something wnet wrong", "error_type" => ""));
        ...
        if (!empty($post_data)) {
            ...
            if (isset($post_data['paymentid']) && isset($post_data['trackid']) && isset($post_data['trandata'])) {
                ...
                if (!empty($is_transaction)) {
                    ...
                    echo json_encode(array("status" => false, "message" => "Invalid transaction id"));
                    die;
                }
            } else {
