...
class My_account extends MY_Controller
{
	...
	public function group_orders()
	{
		...
		$orders = $this->db->get_where('group_orders', array('user_id' => $user, 'parent_id' => NULL, 'is_merged' => 0, 'child !=' => NULL))->result_array();
		$childorders = $this->db->get_where('group_orders', array('user_id' => $user, 'parent_id' => NULL, 'child' => NULL, 'is_merged' => 0))->result_array();

		$this->mViewData['orders'] = $orders;
		$this->mViewData['childorders'] = $childorders;
		...
		if (!empty($this->uid)) {
			...
			$this->mViewData['data'] = $data;
			$this->Urender('group_orders_list', 'udefault');
			// $this->Urender('order_history_new', 'udefault');
			// $this->Urender('order-history', 'udefault', lang('orders'), $data, $parent);
	...
	public function upload_logo()
	{
		...
		if (isset($_FILES['name']['name']) && $_FILES['name']['name'] != '') {
			...
			if ($logo != false) {
				...
				echo json_encode(array("status" => true, "message" => "Image Upload Successfully", "logo" => $logo));
				die;
			} else {
	...
	public function transaction()
	{
		...
		if (!empty($this->uid)) {
			...
			$balance_details = $this->custom_model->my_where('va_transactions', "amount", array('user_id' => $this->uid, 'transaction_type' => 'debit'));
			$used_balance = 0;
