                ...
                if (!empty($this->session->userdata('compare'))) {
                    ...
                    if (!empty($compare)) {
                        ...
                        if ($type == 'add') {
                            ...
                            if (count($cnt) > 3) {
                                echo json_encode(array("status" => false, "message" => ($language == 'ar' ? "You can't add more than 3" : "You can't add more than 3")));
                                die;
                            }
                            ...
                            echo json_encode(array("status" => true, "message" => ($language == 'ar' ? 'Added to compare successfully' : 'Added to compare successfully'), "compare_count" => $compare_count));
                            die;
                        } else {
                            $compare_count = 0;
                            ...
                            echo json_encode(array("status" => true, "message" => ($language == 'ar' ? 'Removed from compare successfully' : 'Removed from compare successfully'), "compare_count" => $compare_count));
                            die;
                        }
                    } else {
                        $cnt[$append] = array('pid' => $pid);
        ...
        if (!empty($post_data['pid'])) {
            ...
            if (!empty($is_product)) {
                if ($country == 'Abu Dhabi') {
                    $is_product[0]['currancy'] = 'AED';
                } else {
                    $is_product[0]['currancy'] = 'BHD';
                }
