...
class Home extends MY_Controller
{
    ...
    public function test()
    {
        ...
        $to = 'ahmedmohyeldean@gmail.com';
        $from = 'info@port10.sa';
        $subject = 'Hello World from CodeIgniter 3!';
        $content = "<strong>This is a test email sent using SendGrid with CodeIgniter 3.</strong>";

        $response = $this->sendgrid_lib->sendEmail($to, $from, $subject, $content);
        echo "<pre>";
        print_r($response);
        exit;
        if ($response) {
    ...
    public function car_data()
    {
        ...
        if (!empty($uid)) {
            ...
            if (!empty($data)) {
                ...
                echo json_encode(array("status" => false, "message" => ($language == 'ar' ? '<h2 class="ac_empty">عربة التسوق فارغة</h2>' : '<h2 class="ac_empty">YOUR SHOPPING CART IS EMPTY</h2>'), 'message_flag' => 'cart_empty'));
                die;
