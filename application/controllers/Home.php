	...
	public function car_data()
	{
		...
		if(!empty($uid))
		{
			...
			if(!empty($data))
			{
					...
					$html_tag.='<div class="singl_prodct_slidr remove_pro2'.$dvalue['key'].'" >';
						...
						$html_tag.='<div class="singl_prodct_slidr_right">';
							...
							$html_tag.='<div class="slide_proodct_qnty">';
								...
								$html_tag.='<input type="text" class="nty_slid_input qty-input2" value="'.$dvalue['c']['qty'].'" id="q'.$dvalue['key'].'" disabled>';
				...
				echo json_encode(array("status"=>true,"message"=>$html_tag,'message_flag'=>'','grand_total'=>$grand_total)); die;
			}else{
				echo json_encode(array("status"=>false,"message"=>($language == 'ar'? '<h2 class="ac_empty">عربة التسوق فارغة</h2>':'<h2 class="ac_empty">YOUR SHOPPING CART IS EMPTY</h2>'),'message_flag'=>'cart_empty')); die;
				
			}
		}else{			
			echo json_encode(array("status"=>false,"message"=>($language == 'ar'? 'الرجاء تسجيل دخول أو إنشاء حساب':'Please Login or create account'),'message_flag'=>'login')); die;
		}
	}
