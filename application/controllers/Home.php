...
class Home extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $featured_data = $this->custom_model->get_data_array("SELECT pro.id,pro.product_name,pro.category,pro.subcategory,pro.price,pro.sale_price,pro.stock_status,pro.stock,pro.product_image,pro.status,pro.seller_id,pro.price_select,pro.brand,pro.unite,pro.min_order_quantity FROM $product as pro INNER JOIN admin_users as admin ON pro.seller_id=admin.id $sub_query GROUP by pro.category ORDER BY RAND() limit 12 ");
        ...
        if (!empty($product_advertise)) {
            ...
            $adds_product = $this->custom_model->get_data_array("SELECT pro.id,pro.product_name,pro.category,pro.subcategory,pro.price,pro.sale_price,pro.stock_status,pro.stock,pro.product_image,pro.status,pro.price_select,pro.seller_id,pro.brand,pro.unite,pro.min_order_quantity FROM $product as pro INNER JOIN admin_users as admin ON pro.seller_id=admin.id $sub_query $order_by ");
        ...
        $this->mViewData['uid'] = $this->session->userdata('uid');
    ...
    public function vendor_info($vendor_id = '')
    {
        ...
        $currency_symbol = $this->return_currency_symbol($currency, $language);
        $this->mViewData['currency'] = $currency;
        $vendor_data = $this->db->get_where('admin_users', array('id' => $vendor_id))->result_array();
        $this->mViewData['vendor_data'] = $vendor_data;
        $this->Urender('vendor_info', 'udefault');
    }

    public function download_catalog($vendor_id = '')
    {
        $language = $this->uri->segment(1);
        ...
        $catalog = $this->db->get_where('catalog', array('seller_id' => $vendor_id, 'is_active' => 1))->result();
        $vendor = $this->db->get_where('admin_users', array('id' => $vendor_id))->result();
        $seller_product_array = json_decode($catalog[0]->seller_products);
        // $seller_products = $this->db->get_where('product',array('id'=>$seller_product_array))->result();
        $this->db->select('product.*');
        $this->db->from('product');
        $this->db->where_in('product.id', $seller_product_array);

        $query = $this->db->get();
        $seller_products = $query->result();
        ...
        $this->mViewData['seller_products'] = $seller_products;
        if ($catalog[0]->theme == 'red') {
            $this->Urender('download_catalog2', 'udefault');
        } else {
            $this->Urender('download_catalog', 'udefault');
        }
    }
    public function catalog_pdf($vendor_id = '')
    {
        $language = $this->uri->segment(1);
        $catalog = $this->db->get_where('catalog', array('seller_id' => $vendor_id))->result();
        $vendor = $this->db->get_where('admin_users', array('id' => $vendor_id))->result();
        $seller_product_array = json_decode($catalog[0]->seller_products);
        // $seller_products = $this->db->get_where('product',array('id'=>$seller_product_array))->result();
        $this->db->select('product.*');
        ...
        $response = $this->pdf_create->get_catalog_pdf($pdf_data, $language);
    }
    ...
    public function checkout($payment_option = '')
    {
        ...
        if (!empty($this->session->userdata('content'))) {
            ...
            if (empty($payment_option)) {
                ...
                if ($payment_option == 'va_transfer' || $payment_option == 'online') {
                } else {
    ...
    public function place_order()
    {
        ...
        // print_r($post_arr);
        // exit();
        if ($post_arr['purchasing_mode'] == 'Single purchasing') {
            if (!empty($post_arr)) {
                if (isset($post_arr['first_name']) && isset($post_arr['last_name']) && isset($post_arr['payment_mode']) && isset($post_arr['mobile_no']) && isset($post_arr['email']) && isset($post_arr['address_1']) && isset($post_arr['country']) && isset($post_arr['city']) && isset($post_arr['state']) && isset($post_arr['pincode']) && isset($post_arr['google_address']) && isset($post_arr['lat']) && isset($post_arr['lng'])) {
                    ...
                    if (!empty($send_data['first_name']) && !empty($send_data['last_name']) && !empty($send_data['payment_mode']) && !empty($send_data['mobile_no']) && !empty($send_data['email']) && !empty($send_data['address_1']) && !empty($send_data['country']) && !empty($send_data['city']) && !empty($send_data['state']) && !empty($send_data['pincode']) && !empty($send_data['pincode'])) {
                        ...
                        if ($order_price > $user_va_balance) {
                            echo json_encode(array("status" => false, "message" => ($language == 'ar' ? "You don't have enough balance, please recharge" : "You don't have enough balance, please recharge"), "flag" => "redirect", "url" => base_url($language . '/home/view_cart')));
        ...
        if ($post_arr['purchasing_mode'] == 'Group purchasing') {
            if (!empty($post_arr)) {
                if (isset($post_arr['first_name']) && isset($post_arr['last_name']) && isset($post_arr['payment_mode']) && isset($post_arr['mobile_no']) && isset($post_arr['email']) && isset($post_arr['address_1']) && isset($post_arr['country']) && isset($post_arr['city']) && isset($post_arr['state']) && isset($post_arr['pincode']) && isset($post_arr['google_address']) && isset($post_arr['lat']) && isset($post_arr['lng'])) {
                    $send_data = array();
                    $send_data['first_name'] = trim($post_arr['first_name']);
                    ...
                    if (!empty($send_data['first_name']) && !empty($send_data['last_name']) && !empty($send_data['payment_mode']) && !empty($send_data['mobile_no']) && !empty($send_data['email']) && !empty($send_data['address_1']) && !empty($send_data['country']) && !empty($send_data['city']) && !empty($send_data['state']) && !empty($send_data['pincode']) && !empty($send_data['pincode'])) {
                        ...
                        $this->load->library('place_order');
                        $response = $this->place_order->create_group_order($send_data, $products, $uid, 'website', $currency, $tax_table);
                        ...
                        if (!empty($response)) {
                            ...
                            if ($send_data['payment_mode'] == 'online') {
                                ...
                                $this->load->library("email_send");
                                if ($language == 'en') {
                                    $this->email_send->send_group_invoice_new_en($response['display_order_id']);
                                } else {
                                    $this->email_send->send_group_invoice_new_ar($response['display_order_id']);
                                }



                                if ($post_arr['pin_order_ids']) {
                                    $elements = explode(',', $post_arr['pin_order_ids']);
                                    $pin_order_id = array_map('intval', $elements);

                                    $pin_order = $this->db->get_where('group_orders', array('id' => $pin_order_id[0]))->result_array();


                                    if ($language == 'en') {
                                        $this->email_send->send_group_invoice_merge_en($pin_order[0]['email'], $response['display_order_id'], $post_arr['pin_order_ids']);
                                    } else {
                                        $this->email_send->send_group_invoice_merge_ar($pin_order[0]['email'], $response['display_order_id'], $post_arr['pin_order_ids']);
                                    }
                                }

                                // $this->email_send->send_invoice($response['display_order_id']);
                                // redirect($language.'/home/thankyou/'.en_de_crypt($response['display_order_id']));
                                echo json_encode(array("status" => true, "message" => ($language == 'ar' ? 'Order Place Successfully' : 'Order Place Successfully'), "flag" => "redirect", "url" => base_url($language . '/home/groupOrderMessage/' . en_de_crypt($response['display_order_id']))));
                                die;
                            }
                        } else {
                            // redirect($language);
    ...
    public function no_group_order_merge_request()
    {
        $language = $this->uri->segments[1];
        ...
        $language = $this->uri->segments[1];
        $data = $_GET;
        $display_new_order_id = $data['display_order_id'];
        $group_order_id = $this->db->get_where('group_orders', array('display_order_id' => $display_new_order_id))->result();


        // print_r($group_order_id[0]->id);                                          
        $merged_orders = $data['pin_order_ids'];
        $elements = explode(',', $merged_orders);
        $pin_order_id = array_map('intval', $elements);
        $parent = json_encode($pin_order_id);
        $parent_order_id = $this->db->get_where('group_orders', array('id' => $pin_order_id[0]))->result();
        $parent_child = json_decode($parent_order_id[0]->child);
        // print_r($parent_child);
        // exit();

        $this->load->library("email_send");
        if ($language == 'en') {
            $this->email_send->send_group_order_merge_request_no_en($display_new_order_id, $pin_order_id[0]);
        } else {
            $this->email_send->send_group_order_merge_request_no_ar($display_new_order_id, $pin_order_id[0]);
        }


        redirect($language);
    }
    public function group_order_merge_request()
    {
        // print_r($_GET);
        $language = $this->uri->segments[1];
        $data = $_GET;
        ...
        // print_r($parent_child);
        // exit();
        if (in_array($group_order_id[0]->id, $parent_child)) {

            if ($language == 'en') {
                $this->session->set_flashdata('login_message', 'Already Merged');
            } else {
                $this->session->set_flashdata('login_message', 'الرجاء الدخول لإنشاء حساب');
            }
            $this->load->library("email_send");
            if ($language == 'en') {
                $this->email_send->send_group_order_merge_request_yes_en($display_new_order_id, $pin_order_id[0]);
            } else {
                $this->email_send->send_group_order_merge_request_yes_ar($display_new_order_id, $pin_order_id[0]);
            }
            redirect($language);
        } else {
            array_push($pin_order_id, $group_order_id[0]->id);
            // $order_child = json_encode($pin_order_id);
            $order_child = '[' . implode(',', $pin_order_id) . ']';
            $sql = "UPDATE group_orders SET child = '$order_child' WHERE id = $pin_order_id[0]";
            $sql1 = "UPDATE group_orders SET parent_id = '$parent' WHERE display_order_id = $display_new_order_id";
            ...
            // $result = json_decode($order_child);
            // echo "Value 17 is not found in the array.";
            $this->load->library("email_send");
            if ($language == 'en') {
                $this->email_send->send_group_order_merge_request_yes_en($display_new_order_id, $pin_order_id[0]);
            } else {
    ...
    public function cronMergeOrders()
    {

        //get all the orders from group_orders table where child is not null and is_merged = 0
        $group_admin_orders = $this->db->get_where('group_orders', array('is_merged' => 0, 'child !=' => NULL))->result();

        foreach ($group_admin_orders as $group_orders) {

            //finding the child order ids
            $child_orders = json_decode($group_orders->child);

            foreach ($child_orders as $child_order) {
                //get the details of the child orders from group_order table
                $order = $this->db->get_where('group_orders', array('id' => $child_order))->result()[0];

                //retrieved data from database of the group_orders
                $order_data = array(
                    ...
                    'transfer_fees' => $order->transfer_fees,
                    'bank_fees' => $order->bank_fees,
                ...
                //finding the items from group_order_items table with respect to the group_orders table
                $group_order_items = $this->db->get_where('group_order_items', array('order_no' => $order->id))->result()[0];

                //inserting the retrieved data in the order_master table as a single order
                $order_master_data = $this->custom_model->my_insert($order_data, 'order_master');

                //retrieved data from the group_order_items table from the database
                $item_data = array(
                    'item_id' => $group_order_items->item_id,
                    'order_no' => $order_master_data,
                    'trans_ref' => $group_order_items->trans_ref,
                    'user_id' => $group_order_items->user_id,
                    'product_id' => $group_order_items->product_id,
                    'seller_id' => $group_order_items->seller_id,
                    'product_name' => $group_order_items->product_name,
                    'quantity' => $group_order_items->quantity,
                    'price' => $group_order_items->price,
                    'shipping_cost' => $group_order_items->shipping_cost,
                    'sub_total' => $group_order_items->sub_total,
                    'tax' => $group_order_items->tax,
                    'commision' => $group_order_items->commision,
                    'created_date' => $group_order_items->created_date,
                    'order_status' => $group_order_items->order_status,
                    'payment_status' => $group_order_items->payment_status,
                    'payment_mode' => $group_order_items->payment_mode,
                    'order_comment' => $group_order_items->order_comment,
                    'delivery_date' => $group_order_items->delivery_date,
                    'attribute' => $group_order_items->attribute,
                    'id_size' => $group_order_items->id_size,
                    'unit' => $group_order_items->unit,
                    'is_delivery_available' => $group_order_items->is_delivery_available,
                    'shipping_id' => $group_order_items->shipping_id,

                );

                //inserting the data in the order_items table as with respect to single order_master data
                $order_items_data = $this->custom_model->my_insert($item_data, 'order_items');

                //deleting the data from the group_orders table after the data is inserted to order_master table
                $this->custom_model->my_delete(array('id' => $child_order), 'group_orders');

                //deleting the data from the group_order_items table after the data is inserted in order_items table
                $this->custom_model->my_delete(array('order_no' => $order->id), 'group_order_items');
            }
        }
    ...
    public function groupOrderMessage($display_order_id = '')
    {
        ...
        if (!empty($display_order_id)) {
            ...
            if (!empty($order_data)) {
                ...
                $order_master_id = $order_data[0]['id'];
                if ($language == 'en') {
                    $order_items = $this->custom_model->get_data_array(" SELECT ois.product_id,ois.product_name,ois.quantity,ois.price,ul.unit_name FROM group_order_items as ois LEFT JOIN unit_list as ul ON ois.unit=ul.id WHERE ois.order_no='$order_master_id' ");
                } else {
                    $order_items = $this->custom_model->get_data_array(" SELECT ois.product_id,ois.product_name,ois.quantity,ois.price,ul.unit_name FROM group_order_items as ois LEFT JOIN unit_list_trans as ul ON ois.unit=ul.id WHERE ois.order_no='$order_master_id' ");
                }
    ...
    public function group_purchasing()
    {
        $data = $adata = array();
        $language = $this->uri->segment(1);
        ...
        $this->mViewData['currency_symbol'] = $currency_symbol;
        // $this->mViewData['shipping_charge']= $this->return_shipping_charge($data,$currency,$tax_table);
        $this->Urender('group_purchasing', 'udefault');
    }
    public function group_purchasing_submit()
    {
        $post_arr = $this->input->post();

        if ($post_arr['orderid_group_by']) {
            $new_order_ids = $post_arr['new_order_ids'];
            $elements = explode(',', $post_arr['orderid_group_by']);
            //@ap@
            $response = $this->db->get_where('group_orders', array('id' => $elements[0]))->result_array();
            // print_r($elements[0]);
            // die;
            // exit();
            $pin_order_id = array_map('intval', $elements);

            array_push($pin_order_id, $post_arr['new_order_ids']);

            $order_child = '[' . implode(',', $pin_order_id) . ']';
            // print_r($order_child);
            // exit();
            $parent = '[' . $pin_order_id[0] . ']';
            $sql = "UPDATE group_orders SET child = '$order_child' WHERE id = $pin_order_id[0]";
            $sql1 = "UPDATE group_orders SET parent_id = '$parent' WHERE id = $new_order_ids";
            // $sql2 = "UPDATE group_orders SET is_merged = 1 WHERE id = $new_order_ids";
            ...
            if ($language == 'en') {
                $this->email_send->send_group_invoice_merge_en($response[0]['email'], $new_order_ids, $elements[0]);
            } else {
                $this->email_send->send_group_invoice_merge_ar($response[0]['email'], $response['display_order_id'], $post_arr['pin_order_ids']);
            }
            redirect($language . '/my_account/group_orders/');
        }
    }
    public function place_order_remove()
    {
        $post_arr = $this->input->post();

        //get the language from uri @ap@
        ...
        if ($post_arr['orderid_group_by']) {
            $parent_order_id = $post_arr['parent_order_id'];
            $orderid_group_by = $post_arr['orderid_group_by'];
            $data_group_orders = $this->db->get_where('group_orders', array('id' => $parent_order_id))->result_array();
            $pin_order_id = json_decode($data_group_orders[0]['child']);
            if (($key = array_search($orderid_group_by, $pin_order_id)) !== false) {
                unset($pin_order_id[$key]);
            }



            // array_push($pin_order_id, $post_arr['parent_order_id']);
            $order_child = '[' . implode(',', $pin_order_id) . ']';
            // $order_child = json_decode($pin_order_id); // Decodes the JSON string into an associative array


            // print_r($order_child);
            // // print_r($order_child);
            // exit();
            $sql = "UPDATE group_orders SET child = '$order_child' WHERE id = $parent_order_id";
            $sql1 = "UPDATE group_orders SET parent_id = NULL WHERE id = $orderid_group_by";
            // $sql2 = "UPDATE group_orders SET is_merged = 0 WHERE id = $new_order_ids";
            ...
            if ($language == 'en') {
                $this->email_send->send_group_order_merge_request_no_en($response[0]['display_order_id'], $post_arr['pin_order_ids']);
            } else {
                $this->email_send->send_group_order_merge_request_no_ar($response[0]['display_order_id'], $post_arr['pin_order_ids']);
            }
            redirect($language . '/my_account/group_orders/');
        }
    }
    public function get_unmerged_check_group_orders()
    {

        $pin_code = $_POST;
        // $pin_code['pin_check'] = 712134;

        $raa = json_decode($_POST['order_id_check'], true);

        $user_id =  $this->session->userdata('uid');
        $relatedOrders = [];
        $this->db->select('*');
        $this->db->from('group_orders');
        $this->db->where_in('id', $raa);
        $query = $this->db->get();
        $rows = $query->result_array();

        foreach ($rows as $row) {

            array_push($relatedOrders, $row);
        }

        echo json_encode($relatedOrders);
        exit();
    }
    public function get_pin_check_group_orders()
    {
        ...
        $query = $this->db->get_where('group_orders', ['pincode' => $pin_code['pin_check'], 'user_id !=' => $user_id]);
        $rows = $query->result_array();

        foreach ($rows as $row) {
            if (!is_null($row['parent_id'])) {
                continue; // Skip the row if parent is not null
            }
            // array_push($relatedOrders, $row);
            if (!is_null($row['child'])) {
                $decodedOrders = json_decode($row['child'], true);

                // Array to store order details
                $ordersArray = [];

                foreach ($decodedOrders as $orderId) {
                    $orderQuery = $this->db->get_where('group_orders', ['id' => $orderId]);
                    $orderDetails = $orderQuery->result_array();

                    if (!empty($orderDetails)) {
                        $ordersArray[] = $orderDetails;
                    }
                }

                array_push($relatedOrders, $ordersArray);
            } else {
                array_push($relatedOrders, $row);
            }
        }
    ...
    public function get_pin_check()
    {
        ...
        $query = $this->db->get_where('group_orders', ['pincode' => $pin_code['pin_check']]);
        $rows = $query->result_array();
