...
class Ajax extends MY_Controller {
	...
	public function shipping_rate()
	{
		...
		if(empty($uid))
		{	
			echo json_encode( array("status" => false,"message" => ($language == 'ar'? 'لرجاء الدخول لإنشاء حساب':'Please Login Or Create Account!!'),"flag"=>"login_message" ) );die;	
		}
				...
				if(!empty($send_data['first_name']) && !empty($send_data['last_name']) && !empty($send_data['payment_mode']) && !empty($send_data['mobile_no']) && !empty($send_data['email']) && !empty($send_data['address_1']) && !empty($send_data['country']) && !empty($send_data['city']) && !empty($send_data['state']) && !empty($send_data['pincode']) && !empty($send_data['pincode']) )
				{
					...
					}else if($post_arr['flow_type']=='invoice')
					{
						...
						if(!empty($post_arr) && $data_res['status']==1 )
						{
							...
							echo json_encode( array("status" => $data_res['status'],"message" =>$data_res['message'] ,"flag"=>"",)); die;
						}						
					}
					if(empty($products))
					{
	...
	public function seach_main_cat_sub_cat()
	{
		$post_data = $this->input->post();
	...
	public function seach_category()
	{
		$post_data = $this->input->post();
			...
			if(!empty($is_vouchers))
			{
				...
				if($is_vouchers[0]['status']==1)
				{
					...
					if($end_date>=$today)
					{
						...
						echo json_encode(array("status"=>true,"discount_value"=>$discount_value,"shipping_flag"=>$is_vouchers[0]['free_shipping'],"cart_total"=>$cart_total));
						die;						
					}else{
						echo json_encode(array("status"=>false,"message"=>"code_expire"));
						die;
					}
				}else{
				echo json_encode(array("status"=>false,"message"=>"code_expire"));
				...
				echo json_encode(array("status"=>false,"message"=>"invalid_code"));
				die;
			}			
		}else{
	...
	public function csv_dwonload_id($api_flag='')
	{
		die;
		// $api_flag=2;
	...
	public function cancel_request()
	{
		...
		if(empty($uid))
		{			
			echo json_encode(array('status'=>false,"message"=>($language == 'ar'? 'الرجاء تسجيل دخول أو إنشاء حساب':'Please Login'))); die;
		}
		$post_data=$this->input->post();
		if(!empty($post_data))
		{
			if(isset($post_data['req_id']))
			{
				...
				if(!empty($is_request))
				{
					...
					{
						echo json_encode(array('status'=>false,"message"=>($language == 'ar'? 'تم إلغاء الطلب':'Request already cancelled'))); die;
					}
					if($is_request[0]['quotation_status']=='Rejected')
					{
						echo json_encode(array('status'=>false,"message"=>($language == 'ar'? "لا يمكن إلغاء طلب مرفوض":"You can't cancelled rejected request"))); die;
					}
					if($is_request[0]['quotation_status']=='Confirmed')
					{
						echo json_encode(array('status'=>false,"message"=>($language == "ar"? "لا يمكن إلغاء طلب مؤكد":"You can't cancelled confirmed request"))); die;
					}					
					...
					if($response)
					{
						...
						echo json_encode(array('status'=>true,"message"=>($language == 'ar'? 'تم إلغاء الطلب بنجاح':'Request cancelled successfully'))); die;
					}else{
	...
	public function switch_seller()
	{
		...
		if(!empty($uid) && !empty($type) )
		{
			if($type=='buyer')
			{
				...
				if(!empty($is_data))
				{
					...
					if(empty($is_data2))
					{
						...
						$this->session->set_userdata($data);
						echo json_encode(array('status'=>true,"message"=>($language == 'ar'? 'تم إنشاء حساب المورد بنجاح':'Seller account created successfully'),'message_flag'=>'')); die;	
					}else{
						echo json_encode(array('status'=>false,"message"=>($language == 'ar'? 'ناك خطأ ما':'Something Went Wrong'),'message_flag'=>'')); die;			
					}
				}else{
					echo json_encode(array('status'=>false,"message"=>($language == 'ar'? 'لا يسمح بالوصول':'No access allowed'),'message_flag'=>'')); die;	
			...
			echo json_encode(array('status'=>false,"message"=>"Please Login OR Create Account",'message_flag'=>'login')); die;
		}
	}
	...
	{	 
		die;
		$query = array();$user_id = "1";$language = "en";
