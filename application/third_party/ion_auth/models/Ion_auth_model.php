...
class Ion_auth_model extends CI_Model
{
    ...
    public function identity_check($identity = '')
    {
        ...
        return FALSE;
        // 05-05-2020 this comment getting error when you login in admin selelct remember me button
    ...
    public function login($identity, $password, $remember = FALSE)
    {
        ...
        $query = $this->db->select($this->identity_column . ', email, id, password, active, last_login, group_id,is_terminate,subs_status,is_email_verify,phone,username,type,access_permission')
            ->where($this->identity_column, $identity)
            ->limit(1)
            ->order_by('id', 'desc')
            ->get($this->tables['users']);
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === TRUE) {
                ...
                if ($user->is_email_verify == 0) {
                    $this->trigger_events('post_login_unsuccessful');
                    $this->set_error('login_unsuccessful_email_verify');
                ...
                if ($user->is_terminate == 1) {
                    $this->trigger_events('post_login_unsuccessful');
                    $this->set_error('login_unsuccessful_terminate');
                ...
                if ($user->group_id == 9) {
                    $this->trigger_events('post_login_unsuccessful');
                    $this->set_error('login_unsuccessful_not_allowed');
                ...
                if ($user->group_id == 5 && $user->subs_status == 'expired') {
                    redirect('en/price');
                }
     ...
     * @author Mathew
     **/
    public function user_login($identity, $password, $type = '', $remember = FALSE)
    {
        // print_r($type);
        $this->trigger_events('pre_login');
        ...
        $users = $this->db->where($this->identity_column, $identity)
        ...
        // echo "<pre>";
        // print_r($users[0]->type); die;
        if (!empty($users)) {
            foreach ($users as $krey => $user) {
                // bcrypt
                if ($this->hash_method == 'bcrypt') {
                    if ($this->bcrypt->verify($password, $user->password)) {
                        ...
                        /*elseif ($users[0]->type !== $type) {
                            return 'error1';
                        }*/
                        $date = date('Y/m/d H:i:s');

                        $this->load->helper('date');
                        $this->db->update('admin_users', array('last_login' => $date), array('id' => $user->id));
                        $this->clear_login_attempts($identity);
                        return array($user);
                    } else {
                        return 'password';
    ...
    public function users($groups = NULL)
    {
        ...
        if (isset($this->_ion_select) && !empty($this->_ion_select)) {
            ...
            // ));
            $this->db->select(array('admin_users' . '.*', 'admin_users' . '.id as id', 'admin_users' . '.id as user_id'
            ));
        }
        ...
        $this->response = $this->db->get('admin_users');
    ...
    public function user($id = NULL)
    {
        ...
        $this->limit(1);
        $this->order_by('admin_users' . '.id', 'desc');
        $this->where('admin_users' . '.id', $id);
    ...
    public function get_users_groups($id = FALSE)
    {
        ...
        // return $this->db->select('admin_users_groups'.'.'.'group_id'.' as id, '.'admin_groups'.'.name, '.'admin_groups'.'.description')
        //                 ->where('admin_users_groups'.'.'.$this->join['users'], $id)
        //                 ->join('admin_groups', 'admin_users_groups'.'.'.'admin_groups'.'='.'id'.'.id')
        //                 ->get('admin_users_groups');

        return $this->db->query("SELECT admin_users_groups.group_id as id, admin_groups.name, admin_groups.description FROM admin_users_groups INNER JOIN admin_groups ON admin_users_groups.group_id=admin_groups.id WHERE admin_users_groups.user_id= '$id' ");
    }
    ...
    public function set_session($user)
    {
        ...
        $session_data = array(
            ...
            'email' => $user->email,
            'user_name' => $user->id, //everyone likes to overwrite id so we'll use user_id
        ...
        if ($user->group_id == 5 || $user->group_id == 10) {
            $front_login = false;
            if ($user->group_id == 5) {
                $front_login = true;
            }
