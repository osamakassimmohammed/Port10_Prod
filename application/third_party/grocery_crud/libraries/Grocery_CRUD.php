		...
		switch ($real_type) {
			...
			case 'upload_file':
				...
				else
				{
					...
					$file_url_anchor = '<a target="_blank" href="'.$file_url.'"';
					if($is_image)
					{
						$file_url_anchor .= ' class="image-thumbnail"><img class="image-thumbnail" src="'.$file_url.'" height="50px">';
					}
					else
					{
...
class grocery_CRUD_Model_Driver extends grocery_CRUD_Field_Types
{
	...
	protected function get_list()
	{
		...
		if(!empty($this->relation))
			foreach($this->relation as $relation)
				$this->basic_model->join_relation($relation[0],$relation[1],$relation[2],$relation[3]);
