...
class Email_send
{
	...
	public function send_group_invoice_merge_en($pin_order_email = '', $display_order_id = '', $pin_order_ids = '')
	{
		...
		// exit();
		$order_master = $this->CI->custom_model->my_where('group_orders', 'id,display_order_id,first_name,last_name,net_total,user_id,currency,payment_mode,email,mobile_no,address_1,city,pincode, state,country', array('display_order_id' => $display_order_id));
		$group_purchase = $this->CI->custom_model->my_where('group_orders', 'first_name, last_name', array('email' => $pin_order_email));
		// print_r($order_master);
		// exit();

		// if ($order_master[0]['payment_mode'] == 'va_transfer') {
		// 	$payment_mode = 'Virtual Account Transfer';
		// } else {
		// 	$payment_mode = 'Credit Card';
		// }
