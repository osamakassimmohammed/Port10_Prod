...
class Form {
	...
	public function field_email($name = 'email', $value = NULL, $extra = array(), $label = '')
	{
		$data = array('type' => 'email', 'id' => $name, 'name'	=> $name, 'placeholder' => $label);
		$value = ($value===NULL) ? $this->get_field_value($name) : $value;
		return form_input($data, $value, $extra, $label);
	}

	// Input field (type = radio)
	public function field_radio($name = 'radio', $value = NULL, $extra = array(), $checked = '', $label = '')
	{
		$data = array('type' => 'radio', 'id' => $value, 'name'	=> $name, 'class' => 'with-gap radio-col-green', 'checked' => $checked);
		if (!empty($extra))
		{
			foreach ($extra as $mkey => $mvalue)
			{
				$data[$mkey] = $mvalue;
			}
		}
		$value = ($value===NULL) ? $this->get_field_value($name) : $value;
		return form_radio($data, $value, $extra, $label);
	}

	// Password field
	public function field_password($name = 'password', $value = NULL, $extra = array(), $label = '')
	{
		$data = array('id' => $name, 'name' => $name, 'placeholder' => $label);
		$value = ($value===NULL) ? '' : $value;
		return form_password($data, $value, $extra, $label);
	}
	...
	public function bs3_phone($label, $name, $value = NULL, $extra = array(), $type = 'text')
	{
		...
		$extra['class'] = 'form-control '.$extra['class'];
		return '<div class="input-group"><div class="form-line">'.$this->field_text($name, $value, $extra, $label, $type).'</div></div>';
	}

	public function bs3_email($label, $name = 'email', $value = NULL, $extra = array())
	{
		$extra['class'] = 'form-control';
		return '<div class="form-group form-float form-group-lg"><div class="form-line">'.$this->field_email($name, $value, $extra, $label).'</div></div>';
	}

	public function bs3_password($label, $name = 'password', $value = NULL, $extra = array())
	{
		$extra['class'] = 'form-control';
		$str = '<div class="form-group form-float form-group-lg"><div class="form-line';
		if (!empty($value)) {	$str .= ' focused '; };
		$str .= '">'.$this->field_password($name, $value, $extra, $label).'</div></div>';
	...
	{
		$extra['class'] = 'form-control';
		return '<div class="form-group form-float">'.form_label($label, $name).$this->field_textarea($name, $value, $extra).'</div>';
	}

	public function bs3_radio($label, $name, $value = NULL, $extra = array(), $checked = '', $type = '')
	{
		// $extra['class'] = '';
		return $this->field_radio($name, $value, $extra, $checked).form_label($label, $value);
	}
