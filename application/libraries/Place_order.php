...
class Place_order
{
	...
	// @ap@
	public function get_user_viban($uid)
	{
		if ($uid) {
			// get user va details
			$user_account_details = $this->CI->custom_model->my_where('account_details', 'id,viban', array('user_id' => $uid))[0];
			//return balance in va
			return ($user_account_details['viban']);
		}
	}
	...
	public function get_order_price($products, $tax_table)
	{
		...
		if (isset($tax_table['cap_rate'])) {
			...
			$get_product = $this->CI->custom_model->my_where('product', 'id,sale_price', array('id' => $products['pid']))[0];
			// get the price of total product excluding estimated vat
			$order_price = (($get_product['sale_price'] * $products['qty']) + $products['shipping_cost']) + $tax_table['cap_rate'];
			// get the amount of estimated vat
			$estimated_vat = ((($get_product['sale_price'] * $products['qty']) + $tax_table['cap_rate']) * $tax_table['vat']) / 100;
